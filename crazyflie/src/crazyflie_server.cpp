...
using crazyflie_interfaces::msg::VelocityWorld_;
...
// ###########################################################
  ...
  void cmd_velocity_world_changed(const crazyflie_interfaces::msg::VelocityWorld::SharedPtr msg) {
    ...
    float vy = msg->vel.y;
    float vz = msg->vel.z;
    // Conversione Rad -> Deg (il firmware Crazyflie solitamente vuole gradi/s per lo yaw rate)
    float yawRate = msg->yaw_rate * 180.0 / M_PI; 
    
    // Questa funzione invia il setpoint di velocità (Body o World frame dipende dal parametro mode nel firmware, 
    // ma sendVelocityWorldSetpoint di solito imposta il mode corretto).
